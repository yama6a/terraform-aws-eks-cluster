...
locals {
  // only instances >= medium (medium, large, 2xlarge ...) support performance insights for MariaDB
...
module "security_group" {
  ...
  tags        = var.tags
  description = "MariaDB DB security group"
  vpc_id      = var.vpc_id
  ...
  ingress_with_source_security_group_id = [
    {
      ...
      description              = "Allow MariaDB TCP Traffic via Ingress Rule from EKS Cluster Nodes"
    },
  ]
}
    ...
    Statement : [
      {
        ...
        Resource = "${module.rds_mariadb.db_instance_arn}/${module.rds_mariadb.db_instance_username}"
        Effect   = "Allow"
